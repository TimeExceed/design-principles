...
\title{\kaishu 陶大讲软件工程：原理}
  \author{陶大}
  \institute{YITU tech}
  \date{Created in July.\ 2019, Updated in May.\ 2021}
...
\begin{frame}
    ...
    \pagequote{
        ...
        or to construct or operate the same with \only<1-3>{full cognizance of their design}\only<4->{\textcolor{orange}{cognizance of their design}};
...
\begin{frame}
    ...
    \note[item]{
        中台的逻辑推到极致就是云。
        中台，按照流行的说法，是把多个相似业务模块从各自业务里剥离出来，集中到一个团队去。
        就此而言，中台有两个根本问题需要回答：
        \begin{enumerate}
            \item 怎样衡量中台团队的价值？
            \item 怎样确定中台模块的演进方向？
        ...
        \begin{itemize}
            \item 货币提供了价值度量。
            \item 货币提供了可粗可细的可比较性。
                尤其货币是权衡大业务-小业务、优先级排序、中台做-业务做这些权衡的工具。
